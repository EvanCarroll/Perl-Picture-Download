# This Makefile is for the dm_download_pictures extension to perl.
#
...
#     DIR => []
#     DISTNAME => q[dm_download_pictures]
#     EXE_FILES => [q[script/dm_download_pictures]]
#     NAME => q[dm_download_pictures]
#     NO_META => q[1]
#     PL_FILES => {  }
#     PREREQ_PM => { Getopt::Long=>q[0], IO::File=>q[0], File::Spec=>q[0], ExtUtils::MakeMaker=>q[6.42], Text::CSV=>q[0], LWP=>q[0] }
...
MAKE = make
FULLEXT = dm_download_pictures
BASEEXT = dm_download_pictures
PARENT_NAME = 
DLBASE = $(BASEEXT)
VERSION_FROM = 
OBJECT = 
...
O_FILES  = 
H_FILES  = 
MAN1PODS = script/dm_download_pictures
MAN3PODS = 
...
DISTNAME = dm_download_pictures
DISTVNAME = dm_download_pictures-
...
manifypods : pure_all  \
	script/dm_download_pictures
	$(NOECHO) $(POD2MAN) --section=$(MAN1EXT) --perm_rw=$(PERM_RW) \
	  script/dm_download_pictures $(INST_MAN1DIR)/dm_download_pictures.$(MAN1EXT) 
...
EXE_FILES = script/dm_download_pictures

pure_all :: $(INST_SCRIPT)/dm_download_pictures
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/dm_download_pictures 

$(INST_SCRIPT)/dm_download_pictures : script/dm_download_pictures $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/dm_download_pictures
	$(CP) script/dm_download_pictures $(INST_SCRIPT)/dm_download_pictures
	$(FIXIN) $(INST_SCRIPT)/dm_download_pictures
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/dm_download_pictures
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <AUTHOR>Evan Carroll &lt;evan@dealermade.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Text-CSV" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
